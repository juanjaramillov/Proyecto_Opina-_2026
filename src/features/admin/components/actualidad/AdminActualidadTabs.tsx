        ...
        return (
          <button
            ...
            className={`relative flex flex-col justify-center p-4 rounded-2xl text-left transition-all duration-300 border ${
              active 
                ? 'bg-white border-brand shadow-[0_8px_20px_-6px_rgba(59,130,246,0.3)] ring-1 ring-brand/50 z-10' 
                : 'bg-white/50 border-slate-200 hover:bg-white hover:border-slate-300 hover:shadow-sm'
            }`}
          >
            <div className="flex items-center justify-between w-full mb-1">
              <div className="flex items-center gap-2">
                <div className={`p-2 rounded-xl shrink-0 ${active ? 'bg-brand/10 text-brand' : 'bg-slate-100 text-slate-500'}`}>
                  <Icon className="w-5 h-5" />
                </div>
                <span className={`font-black text-sm leading-tight truncate ${active ? 'text-brand' : 'text-slate-700'}`}>
                  {tab.label}
                </span>
              </div>
              {active && (
                <div className="bg-brand/20 text-brand text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md shrink-0 ml-1">
                  {loading ? '...' : totalTopics}
                </div>
              )}
            </div>

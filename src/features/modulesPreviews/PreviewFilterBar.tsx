        ...
        <div className="space-y-6 mb-8">
            ...
            <div className="flex flex-wrap gap-2">
                {categories.map((cat, i) => (
                    <div key={i} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border transition cursor-default ${i === 0 ? 'bg-brand/10 border-brand/30 text-brand' : 'bg-white border-slate-100 text-slate-400 opacity-60'}`}>
                        {cat}
                    </div>
                ))}
            </div>

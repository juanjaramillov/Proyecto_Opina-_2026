...
export default function TorneoView({ battles, onBack }: TorneoViewProps) {
    ...
    return (
        <div className="space-y-8 flex flex-col animate-in fade-in duration-500 w-full">
            <div className={`w-[100vw] relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] min-h-[80vh] md:min-h-[85vh] ${isGoldenHour ? 'bg-warning-50/30' : 'bg-slate-50 md:bg-transparent'} flex flex-col animate-in fade-in zoom-in-95 duration-500 order-1 border-y border-slate-100 md:border-none shadow-sm md:shadow-none overflow-hidden`}>
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-0 md:p-6 w-full max-w-6xl mx-auto">
                    <div className="w-full max-w-4xl bg-white md:bg-transparent md:border-0 md:shadow-none min-h-[600px] flex flex-col relative mx-auto">
                        <div className="absolute inset-0 pointer-events-none opacity-30">
                            <div className={`absolute top-0 right-0 w-96 h-96 bg-gradient-to-br transition-all duration-700 ${selectedTheme !== 'mix' ? PARENT_INDUSTRIES[selectedTheme]?.theme?.bgGradient : 'from-brand-50 to-white'} rounded-full blur-3xl -translate-y-1/2 translate-x-1/2`} />
                        </div>
                            ...
                            onVote={async (battle_id, option_id, opponentId, metadata) => {
                                try {
                                    ...
                                    if (msg.includes('RATE_LIMITED')) {
                                        showToast("Vas muy rápido. Espera un momento antes de la siguiente ronda.", "info");
                                        throw err;
                                    }

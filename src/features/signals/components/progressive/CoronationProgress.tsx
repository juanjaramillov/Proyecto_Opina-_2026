                        ...
                        <div key={slotIndex} className="relative">
                            <div
                                className={`w-8 h-3 rounded-full transition-all duration-500 border
                                    ${isFilled ? 'bg-blue-400 border-blue-600 shadow-[0_0_10px_rgba(251,191,36,0.5)]' : 'bg-slate-100 border-slate-200'}
                            ...
                            {slotIndex === goal - 1 && (
                                <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-slate-300">
                                    <span className={`material-symbols-outlined text-sm ${isFilled ? 'text-blue-600' : ''}`}>crown</span>
                                </div>
                            )}
                        </div>
                    );

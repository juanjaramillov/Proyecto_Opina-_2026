    ...
    <section className="w-full bg-slate-50 py-16 md:py-24 border-b border-slate-100">
      <div className="max-w-[1400px] mx-auto px-6 md:px-12">
        ...
        <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h2 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tight">Qué está ganando hoy</h2>
            <p className="mt-3 md:text-lg text-slate-500 font-medium">Las narrativas que dominan cada territorio en tiempo real.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {ranking.map((item, idx) => (
            <div key={idx} className="bg-white rounded-[2.5rem] p-8 md:p-10 border border-slate-200 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col hover:-translate-y-1">
              <span className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 block">
                {item.category}
              </span>
              <h3 className="text-2xl md:text-3xl font-black text-slate-900 leading-tight mb-8 flex-grow">
                {item.leader}
              </h3>

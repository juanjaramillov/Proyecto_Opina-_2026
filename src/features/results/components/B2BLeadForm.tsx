        ...
        <form onSubmit={handleSubmit} className="bg-white border border-stroke rounded-[2rem] p-8 md:p-10 text-left w-full mx-auto relative overflow-hidden shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div className="space-y-2">
                    ...
                    <input required type="text" value={name} onChange={e => setName(e.target.value)} className="w-full bg-surface2 border border-stroke rounded-xl px-4 py-3 text-ink focus:outline-none focus:border-brand focus:bg-white transition-colors placeholder:text-slate-500/50" placeholder="Ej. Ana Pérez" />
                </div>
                <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Correo Corporativo</label>
                    <input required type="email" value={email} onChange={e => setEmail(e.target.value)} className="w-full bg-surface2 border border-stroke rounded-xl px-4 py-3 text-ink focus:outline-none focus:border-brand focus:bg-white transition-colors placeholder:text-slate-500/50" placeholder="ana@empresa.com" />
                </div>
                <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Empresa</label>
                    <input required type="text" value={company} onChange={e => setCompany(e.target.value)} className="w-full bg-surface2 border border-stroke rounded-xl px-4 py-3 text-ink focus:outline-none focus:border-brand focus:bg-white transition-colors placeholder:text-slate-500/50" placeholder="Tu Empresa" />
                </div>
                <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Cargo</label>
                    <input required type="text" value={role} onChange={e => setRole(e.target.value)} className="w-full bg-surface2 border border-stroke rounded-xl px-4 py-3 text-ink focus:outline-none focus:border-brand focus:bg-white transition-colors placeholder:text-slate-500/50" placeholder="Ej. Director de Marketing" />
                </div>
            </div>

            <div className="space-y-2 mb-8 relative">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Interés Principal</label>
                <div className="relative">
                    <select required value={interest} onChange={e => setInterest(e.target.value)} className="w-full bg-surface2 border border-stroke rounded-xl px-4 py-3 text-ink focus:outline-none focus:border-brand focus:bg-white transition-colors appearance-none pr-10">

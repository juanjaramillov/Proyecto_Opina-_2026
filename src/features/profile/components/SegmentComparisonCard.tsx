    ...
    return (
        <div className="flex justify-between items-center text-[10px] py-2">
            ...
            <div className="flex items-center gap-2">
                ...
                <span className={`flex items-center font-bold px-1.5 py-0.5 rounded text-[9px] border ${isAbove ? 'text-accent bg-accent/10 border-accent/20' : isBelow ? 'text-brand bg-brand/10 border-brand/20' : 'text-slate-500 bg-surface2 border-stroke'
                    }`}>
                    {isAbove ? '↑' : isBelow ? '↓' : '='}
...
const SegmentComparisonCard: React.FC<Props> = ({ data }) => {
    return (
        <motion.div
            ...
            className="card p-5 group overflow-hidden relative shadow-sm hover:border-brand/20 transition-all border border-stroke bg-white"
        >
            {/* Coherence Badge */}
            <div className="absolute top-0 right-0 p-4">
                <div className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest border ${data.coherence_level === 'Alta' ? 'bg-accent/10 text-accent border-accent/20' :
                    data.coherence_level === 'Media' ? 'bg-surface2 text-slate-600 border-stroke' :
            ...
            <div className="mb-4 pt-2">
                <h4 className="text-lg font-black text-ink group-hover:text-brand transition-colors tracking-tight">
                    {data.entity_name}
                </h4>
                <div className="mt-2 inline-flex items-baseline gap-1.5">
                    ...
                    <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Tu Nota</span>
                </div>
            </div>

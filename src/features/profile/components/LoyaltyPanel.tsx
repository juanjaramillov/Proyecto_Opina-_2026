            ...
            <div className="relative z-10">
                <div className="flex flex-col gap-1 mb-6">
                    <h3 className="text-sm font-black text-ink uppercase tracking-wider flex items-center gap-2">
                        <span className="material-symbols-outlined text-brand text-lg">workspace_premium</span>
                        Tu Nivel Opina+
                    </h3>
                    <p className="text-xs text-slate-600 font-medium">Cada señal te hace avanzar de nivel.</p>
                </div>
                ...
                <div className="bg-surface2 border border-stroke rounded-2xl p-5 mb-6 flex items-center gap-4 group-hover:bg-white transition-colors">
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 shadow-sm border ${current.badgeClass === 'bg-white text-slate-400 border border-slate-200' ? 'bg-white text-slate-500 border-stroke' : current.badgeClass}`}>
                        <span className="material-symbols-outlined text-2xl">trophy</span>
                    </div>
                    <div>
                        <div className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-0.5">Nivel Actual</div>
                    ...
                    <div className="flex items-center justify-between text-[11px] font-bold text-slate-500 mb-3">
                        {next ? (
                            <>
                                ...
                                <span className="text-brand bg-brand/10 px-2 py-0.5 rounded-md border border-brand/20">Faltan {formatPoints(missing)} señales</span>
                            </>
                        ) : (
                ...
                <div className="space-y-6">
                    <div>
                        <div className="text-[10px] uppercase tracking-widest font-black text-slate-500 mb-4 border-b border-stroke pb-2">
                            Beneficios Desbloqueados
                        </div>
                        <ul className="space-y-2.5">
                            {current.benefits.map((b) => (
                                <li key={b} className="text-sm font-bold text-ink flex gap-3 items-center">
                                    <span className="material-symbols-outlined text-[18px] text-accent shrink-0">check_circle</span>
                                    {b}
                                </li>
                            ))}
                        </ul>
                        ...
                        <div className="pt-2">
                            ...
                            <ul className="space-y-2.5">
                                {current.upcomingBenefits.map((b) => (
                                    <li key={b} className="text-sm font-medium text-slate-600 flex gap-3 items-center">
                                        <span className="material-symbols-outlined text-[18px] text-slate-500 shrink-0">lock</span>
                                        {b}
                                    </li>
                                ))}
                            </ul>

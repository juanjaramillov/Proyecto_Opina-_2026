...
type ModuleType = 'versus' | 'torneo';

const Rankings: React.FC = () => {
    const initialParams = new URLSearchParams(window.location.search);
    const initialCategory = initialParams.get("category") || "streaming";
    const initialModule = (initialParams.get("module") === "torneo" ? "torneo" : "versus") as ModuleType;
        ...
        <div className="container-ws section-y space-y-8 pb-32">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex bg-slate-200/50 p-1 rounded-xl shrink-0">
                    ...
                    <button
                        onClick={() => setModuleType('torneo')}
                        className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${moduleType === 'torneo' ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Torneo
                    </button>
                </div>

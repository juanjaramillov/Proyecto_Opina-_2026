...
import LugarSignalWizard from "./LugarSignalWizard";
...
interface LugarDetailViewProps {
    place: Place;
    onClose: () => void;
}

export default function LugarDetailView({ place, onClose }: LugarDetailViewProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center pointer-events-none">
            ...
            >
                ...
                <div className="relative w-full h-72 sm:h-80 shrink-0 bg-slate-800">
                    ...
                    <div className="absolute bottom-0 left-0 w-full p-6 sm:p-8 flex flex-col items-start text-white">
                        ...
                        <div className="flex flex-wrap items-center gap-2 mt-3 text-sm font-semibold text-white/90">
                            <span>{place.tags.split(" • ")[0] || place.category}</span>
                            <span className="text-white/40">•</span>
                            <span>{place.tags.split(" • ")[1] || place.subcategory}</span>
                            <span className="text-white/40">•</span>
                            <span className="flex items-center gap-1"><span className="material-symbols-outlined text-[14px]">location_on</span> {place.distance}</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="p-6 sm:p-8 space-y-8">
                        ...
                        <div className="flex gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-100 items-stretch">
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center py-2 border-r border-slate-200/60">
                                ...
                                <span className="text-xs font-bold text-slate-500 mt-3">Opiniones</span>
                            </div>
                                ...
                                <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                                    ...
                                    <div className="flex flex-col items-center justify-center gap-1.5 bg-blue-50 text-blue-700 p-3 rounded-xl border border-blue-100/50">
                                        ...
                                        <div className="flex flex-col items-center">
                                            ...
                                            <span className="text-lg font-black leading-none mt-0.5">4.8</span>
                                        </div>
                                    </div>
                                    <div className="flex flex-col items-center justify-center gap-1.5 bg-slate-50 text-slate-600 p-3 rounded-xl border border-slate-200/50">
                                        ...
                                        <div className="flex flex-col items-center">
                                            <span className="text-[10px] font-bold uppercase opacity-80">Precios</span>
                                            <span className="text-lg font-black leading-none mt-0.5">3.5</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                {/* Wizard Overlay (Framer Motion AnimatePresence) */}
                <AnimatePresence>
                    {showWizard && (
                        <LugarSignalWizard 
                            place={place} 

...
const HIGHPRIO_JSON = path.join(NEXT_DIR, 'high-priority-review.json');
const HIGHPRIO_HTML = path.join(NEXT_DIR, 'high-priority-gallery.html');
...
async function main() {
  ...
  // 1. Filtrar solo prioridad 'alta'
  const highPrio = overrides.filter(o => o.manualPriority === 'alta');
...
<body>
  <h1>Revisión de Prioridad Alta (${reviewItems.length} assets)</h1>
  <p>Edite <code>docs/catalog/logo-overrides-high-priority.csv</code> para aprobar los casos listados aquí.</p>
  <div class="grid">
`;
...
`;
  ...
  console.log(`📂 CSV exclusivo para edición: docs/catalog/logo-overrides-high-priority.csv`);
  console.log(`📂 Galería visual: public/logos/entities_next/high-priority-gallery.html`);
  console.log(`📂 Reporte JSON: public/logos/entities_next/high-priority-review.json`);

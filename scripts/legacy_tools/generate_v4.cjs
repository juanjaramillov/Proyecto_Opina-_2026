...
const destBase = path.join(process.cwd(), 'opina-results-context-export-v4-true-100');
...
// Include global system structural files
const globalFiles = [
  'src/index.tsx',
  'src/main.tsx',
  'src/index.css',
...
function analyzeEntryPoint(ep) {
  ...
  function scan(filePath) {
    ...
    const importRegex = /(?:import|export)\s+.*?from\s+['"]([^'"]+)['"]|import\s*\(\s*['"]([^'"]+)['"]\s*\)|import\s+['"]([^'"]+)['"]/g;
...
// 4. Copy screenshots
if (fs.existsSync('opina-results-context-export-v2/screenshots')) {
...
console.log(`V4 Copied! Extracted ${copiedFilesCount} source files.`);

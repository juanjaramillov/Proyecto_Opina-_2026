-- FIX: Remove strict 20-vote limit for registered users.
-- The previous schema applied a 20-vote limit to ANY user where is_identity_verified = false.
-- This incorrectly penalized registered, logged-in users who simply had not performed SMS/KYC verification yet,
-- effectively treating them as guests and throwing 'SIGNAL_LIMIT_REACHED'.
-- We now only apply the 20-vote limit strictly to anonymous guests.
...
-- Apply same fix to insert_depth_answers

...
create index if not exists idx_depth_option
on public.depth_answers_structured(option_id);

create index if not exists idx_depth_user
on public.depth_answers_structured(user_id);

create index if not exists idx_profiles_segment
on public.profiles(gender, age_bucket, region);

-- ==========================================
-- MULTI-OPTION SUPPORT (SCALABILITY)
-- ==========================================

create or replace function public.get_depth_multi_option(
    p_option_ids uuid[],
    p_gender text default null,
    p_age_bucket text default null,
    p_region text default null
...
as $$
    ...
    join public.profiles p
        on p.id = das.user_id
    where das.option_id = any(p_option_ids)
      and (p_gender is null or p.gender = p_gender)
      and (p_age_bucket is null or p.age_bucket = p_age_bucket)
      and (p_region is null or p.region = p_region)
    group by das.option_id, das.question_key;
...
grant execute on function public.get_depth_multi_option(uuid[], text, text, text)
to authenticated;

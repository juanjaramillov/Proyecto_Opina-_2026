// Original path: src/features/feed/components/BatchSessionResults.tsx
                ...
                >
                    ...
                    <div className="flex flex-col items-center text-center mb-8 pt-4">
                        <motion.div 
                            ...
                            className="w-20 h-20 rounded-full bg-gradient-to-br from-emerald-400 to-cyan-500 text-white flex items-center justify-center mb-6 shadow-lg shadow-emerald-500/20"
                        >
                            <span className="material-symbols-outlined text-4xl">check_circle</span>
                        </motion.div>
                        <h2 className="text-3xl md:text-4xl font-black text-ink tracking-tight mb-2">¡Misión <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 to-cyan-400">Cumplida</span>!</h2>
                    ...
                    <div className="space-y-4 mb-8">
                        {batchSessionHistory.map((h, i) => {
                            ...
                            return (
                                ...
                                >
                                    <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-gradient-to-r from-blue-500/5 to-cyan-400/5 pointer-events-none" />
                                    
                                    <div className="flex items-center justify-between z-10">
                                        <div className="text-[10px] md:text-xs font-black uppercase tracking-widest text-slate-400">{h.battle.title}</div>
                                        <div className={`px-2.5 py-1 rounded-full text-[10px] font-bold flex items-center gap-1.5 shadow-sm border ${isMajority ? 'bg-blue-50 text-blue-600 border-blue-100' : 'bg-amber-50 text-amber-600 border-amber-100'}`}>
                                    ...
                                    <div className="flex flex-col sm:flex-row items-center gap-3 z-10 w-full mt-1">
                                        <div className={`flex-1 w-full flex flex-col p-3 rounded-xl border ${isMajority ? 'border-blue-200 bg-blue-50/50' : 'border-amber-200 bg-amber-50/50'} relative overflow-hidden`}>
                                            <div 
                                                className={`absolute left-0 bottom-0 h-1.5 transition-all duration-1000 ease-out flex items-center justify-end pr-1 ${isMajority ? 'bg-gradient-to-r from-blue-500 to-cyan-400' : 'bg-amber-400'}`}
                                            ...
                                            <div className="flex justify-between items-end mb-1">
                                                ...
                                                <span className={`text-[11px] font-black ${isMajority ? 'text-blue-600' : 'text-amber-600'}`}>{Math.round(myPct)}%</span>
                                            </div>
                                            <span className={`text-[10px] font-bold ${isMajority ? 'text-blue-500/70' : 'text-amber-500/70'}`}>Tu Selección</span>
                                        </div>

    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {gridTopics.map((topic, i) => (
            <motion.div
                ...
                className="bg-white rounded-2xl md:rounded-3xl border border-slate-200 hover:border-blue-200 hover:shadow-[0_8px_30px_-12px_rgba(37,99,235,0.15)] hover:-translate-y-1 cursor-pointer transition-all duration-300 flex flex-col group relative overflow-hidden"
            >
                {topic.has_answered && (
                ...
                {Boolean(topic.metadata?.image_url) && (
                    <div className="w-full h-48 sm:h-56 bg-slate-100 relative overflow-hidden shrink-0">
                        <img 
                        ...
                        <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between gap-3">
                            <span className="text-[10px] font-bold uppercase tracking-wider text-white bg-blue-600/90 backdrop-blur-sm shadow-sm px-2.5 py-1 rounded-md">
                                {topic.category}
                            </span>
                            {topic.source_domain && (
                                <span className="text-[10px] font-bold text-white/90 drop-shadow-md truncate max-w-[120px]">
                                    {topic.source_domain}
                                </span>
                            )}
                        </div>
                ...
                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-blue-500 to-emerald-400 opacity-0 group-hover:opacity-100 transition-opacity z-20"></div>
